...
pub const Batch = struct {
    ...
    pub fn init(ctx: uph.Context.Context, pipeline_handle: u32, camera: *uph.uph3d.Camera.Camera) !*Batch {
        ...
        b.* = Batch{
            ...
            .draw_cmd_buf = try ctx.renderer().newCommandBuffer(),
            .upload_cmd_buf = try ctx.renderer().newCommandBuffer(),
            .atlas = null,
            .camera = camera,
        };
        return b;
    ...
    pub fn draw(self: *Batch, delta_t: f32) !void {
        ...
        try self.ctx.renderer().submitCommandBuffer(self.upload_cmd_buf);
