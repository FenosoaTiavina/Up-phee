...
pub const Batch = struct {
    ...
    pub fn draw(self: *Batch, delta_t: f32) !void {
        ...
        //
        // Upload vertices
        try self.ctx.renderer().uploadToGPU(
            copy_pass.?,
        ...
        try self.ctx.renderer().uploadToGPU(
            copy_pass.?,
            self.transfer_buffer,
            @intCast(self.vertices.items.len * @sizeOf(Objects.Vertex) + self.indices.items.len * @sizeOf(Objects.Index)),
            u16,
            self.indices.items,
            self.gpu.ibo,
        );
        ...
        try self.ctx.renderer().bindPipeline(render_pass, self.pipeline_handle);
