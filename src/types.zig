...
const math = std.math;
const zmath = @import("uph.zig").zmath;

const minAndMax = @import("uph.zig").Maths.minAndMax;

pub const Vec2_f32 = @Vector(2, f32);
pub const Vec2_usize = @Vector(2, usize);

pub const Vec3_f32 = @Vector(3, f32);
pub const Vec4_u8 = @Vector(4, u8);
pub const Vec4_f32 = @Vector(4, f32);

pub const Mat4_f32 = [4]Vec4_f32;
...
pub const Rectangle = extern struct {
    ...
    pub inline fn getSize(r: Rectangle) Point {
    ...
    pub inline fn hasIntersection(r: Rectangle, b: Rectangle) bool {
        if (c.sdl.SDL_HasIntersectionF(@ptrCast(&r), @ptrCast(&b)) == 1) {
            return true;
        }
        return false;
    }
    ...
    pub inline fn intersectRect(r: Rectangle, b: Rectangle) ?Rectangle {
        ...
        if (c.sdl.SDL_IntersectFRect(@ptrCast(&r), @ptrCast(&b), @ptrCast(&result)) == 1) {
            return result;
        }
        return null;
    }
    ...
    pub inline fn intersectLine(r: Rectangle, p0: *Point, p1: *Point) bool {
        if (c.sdl.SDL_IntersectFRectAndLine(@ptrCast(&r), &p0.x, &p0.y, &p1.x, &p1.y) == 1) {
            return true;
        }
        return false;
    }

    pub inline fn intersectCircle(r: Rectangle, circle: Circle) bool {
        return circle.intersectRect(r);
    }
...
pub const Circle = extern struct {
    ...
    pub inline fn translate(circle: Circle, v: [2]f32) Circle {
        return .{
            .center = circle.center.add(v),
            .radius = circle.radius,
        };
    }

    pub inline fn containsPoint(circle: Circle, p: Point) bool {
        const v: @Vector(2, f32) = .{ circle.center.x - p.x, circle.center.y - p.y };
        return @reduce(.Add, v * v) < circle.radius * circle.radius;
    }
    ...
    pub inline fn intersectRect(circle: Circle, r: Rectangle) bool {
        if (circle.containsPoint(.{ .x = r.x, .y = r.y }) or
            circle.containsPoint(.{ .x = r.x + r.width, .y = r.y }) or
            circle.containsPoint(.{ .x = r.x + r.width, .y = r.y + r.height }) or
            circle.containsPoint(.{ .x = r.x, .y = r.y + r.height }))
        {
            return true;
        }
        return false;
    ...
    pub inline fn intersectTriangle(circle: Circle, t: Triangle) bool {
        if (circle.containsPoint(t.p0) or circle.containsPoint(t.p1) or circle.containsPoint(t.p2)) {
            return true;
        }
        return false;
    }
...
pub const Color = extern struct {
    ...
    inline fn getPixelFormat() [*c]c.sdl.SDL_PixelFormat {
        const S = struct {
            var pixel_format: ?[*c]c.sdl.SDL_PixelFormat = null;
        };
        if (S.pixel_format == null) {
            S.pixel_format = c.sdl.SDL_AllocFormat(c.sdl.SDL_PIXELFORMAT_RGBA32);
        }
    ...
    pub inline fn fromRGBA32(i: u32) Color {
        ...
        c.sdl.SDL_GetRGBA(i, getPixelFormat(), &col.r, &col.g, &col.b, &col.a);
        return col;
    }

    pub inline fn toRGBA32(col: Color) u32 {
        return c.sdl.SDL_MapRGBA(getPixelFormat(), col.r, col.g, col.b, col.a);
    }
    ...
    /// Convert to HSL
    pub inline fn toHSL(co: Color) [4]f32 {
        const hsl = zmath.rgbToHsl(
            zmath.f32x4(
                @as(f32, @floatFromInt(co.r)) / 255,
                @as(f32, @floatFromInt(co.g)) / 255,
                @as(f32, @floatFromInt(co.b)) / 255,
                @as(f32, @floatFromInt(co.a)) / 255,
            ),
        );
    ...
    pub inline fn fromInternalColor(col: u32) Color {
        return .{
            .r = @as(u8, col & 0xff),
            .g = @as(u8, (col >> 8) & 0xff),
            .b = @as(u8, (col >> 16) & 0xff),
    ...
    pub inline fn toInternalColor(col: Color) u32 {
        return @as(u32, col.r) |
            (@as(u32, col.g) << 8) |
            (@as(u32, col.b) << 16) |
            (@as(u32, col.a) << 24);
